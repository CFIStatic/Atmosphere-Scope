...
A proprietary sketch file that does not publish an open schema is not imported. PDF reports and image exports are not turned into wall lengths. A CSV, DXF, or SVG is read only when it matches the rules above.
